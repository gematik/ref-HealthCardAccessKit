...
public class CardSimulatorTerminalResource {
    ...
    public func startUp(wait flag: Bool = true, manager: SimulationManagerType = SimulationManager.shared) throws {
        runner = try manager.createSimulation(
                ...
                preprocessor: self.configManipulators,
...
extension CardSimulatorTerminalResource: CardReaderControllerDelegate {
    public func cardReader(controller: CardReaderControllerType, didConnect reader: CardReaderType) {
    ...
    public func cardReader(controller: CardReaderControllerType, didDisconnect reader: CardReaderType) {
...
class DisconnectedTerminal: CardReaderType {
    let name: String
    var cardPresent: Bool { return false }
    let card: CardType

    init(reader: CardReaderType) {
        self.name = "Disconnected: \(reader.name)"
        self.card = DisconnectedCard()
    }
    ...
    func connect(protocol: CardProtocol) throws -> CardType {
        return card
    }
}
